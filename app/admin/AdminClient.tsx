  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const rows = j?.data ?? [];
        setLocations(rows.map((r: any) => r.name));
      } catch (e) {
        // ignore
      }
    })();
          ...
          {tab === "users" ? (
            ...
            <>
              <div style={{ marginTop: 14 }}>
                <h2 style={{ fontSize: 18, fontWeight: 800, marginBottom: 8, color: "#000" }}>Settings • Locations</h2>
                <p style={{ marginTop: 0, marginBottom: 8, color: "#374151" }}>Manage available locations used by user profiles.</p>
                <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                  ...
                  />
                  <button onClick={addLocation} style={{ padding: "8px 12px", background: "#367C2B", color: "#FFC72C", borderRadius: 8, border: "2px solid #FFC72C", cursor: "pointer" }}>
                    Add Location
                  </button>
                </div>
                  ...
                  {locations.length === 0 ? (
                    ...
                    locations.map((l) => (
                      <div key={l} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fff", padding: 8, borderRadius: 8, border: "1px solid rgba(0,0,0,0.06)" }}>
                        <div>{l}</div>
                        <button onClick={() => deleteLocation(l)} style={{ background: "transparent", border: "none", color: "#dc2626", cursor: "pointer", fontWeight: 700 }}>Delete</button>
                      </div>
                    ))
                  )}
                </div>
