        ...
        {online && planter && (
          <>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              <StatusCard label="HEIGHT" value={planter.height ?? "--"}
                highlight={planter.height === "DOWN"} />
              <StatusCard label="ARMED"
                value={planter.output_on ? "TRIGGERED" : planter.armed ? "ARMED" : "NOT ARMED"}
                alarm={!!planter.output_on} ok={!!planter.armed && !planter.output_on}
                warn={!planter.armed && !planter.output_on} />
              <StatusCard label="SENTINEL"
                value={!planter.sentinel_en ? "OFF" : !planter.armed ? "NOT ARMED" : planter.sentinel_alarm ? "ALARM" : "OK"}
                alarm={!!planter.sentinel_alarm} ok={planter.sentinel_en === true && !!planter.armed && !planter.sentinel_alarm}
                warn={planter.sentinel_en === true && !planter.armed} dim={!planter.sentinel_en} />
              <StatusCard label="SEEDING"
                value={!planter.seed_en ? "OFF" : !planter.armed ? "NOT ARMED" : planter.seed_fault ? `ROW ${planter.seed_fault_row}` : "OK"}
                alarm={!!planter.seed_fault} ok={planter.seed_en === true && !!planter.armed && !planter.seed_fault}
                warn={planter.seed_en === true && !planter.armed} dim={!planter.seed_en} />
              <StatusCard label="VACUUM"
                value={!planter.vac_en ? "OFF" : !planter.armed ? "NOT ARMED" : planter.vac_fault ? "FAULT" : "OK"}
                alarm={!!planter.vac_fault} ok={planter.vac_en === true && !!planter.armed && !planter.vac_fault}
                warn={planter.vac_en === true && !planter.armed} dim={!planter.vac_en} />
            </div>

            {/* Sentinel rates */}
            {planter.sentinel_en && (
              <div className="grid grid-cols-2 gap-3 mb-4">
                <StatusCard label="TARGET RATE"
                  value={planter.sentinel_target_gal != null ? `${planter.sentinel_target_gal.toFixed(2)} gal/ac` : "--"} />
                <StatusCard label="AVG ACTUAL"
